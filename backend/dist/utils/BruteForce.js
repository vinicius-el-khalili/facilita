...
const getGraph = (coordinatesList) => {
    let graph = [];
...
let shortestPath = [];
const BruteForce = (graph, visitedNodes, currentNode, totalPathDistance) => {
    // Exit condition
    ...
    for (let candidateNode = 0; candidateNode < graph.length; candidateNode++) {
        if (!visitedNodes.includes(candidateNode)) {
            ...
            BruteForce(graph, [...visitedNodes], candidateNode, totalPathDistance + graph[currentNode][candidateNode]);
            visitedNodes.pop();
        }
    }
};
